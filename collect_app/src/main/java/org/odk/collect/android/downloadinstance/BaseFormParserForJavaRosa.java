...
public class BaseFormParserForJavaRosa {
  ...
  private static final String t ="BaseFormParserForJavaRosa";
  ...
  public static String xmlWithTimestampComment(String xmlWithoutTimestampComment, String serverUrl) {
    ...
    return xmlWithoutTimestampComment.substring(0, idx) + ODK_TIMESTAMP_COMMENT
        + org.odk.collect.android.downloadinstance.utils.WebUtils.iso8601Date(new Date()) + " on " + serverUrl + " -->"
  ...
  protected BaseFormParserForJavaRosa(String existingXml, String existingTitle, boolean allowLegacy)
      throws ODKIncompleteSubmissionData {
    if (existingXml == null) {
      throw new ODKIncompleteSubmissionData(ODKIncompleteSubmissionData.Reason.MISSING_XML);
    }
    ...
    try {
      ...
      throw new ODKIncompleteSubmissionData(
          ...
          ODKIncompleteSubmissionData.Reason.BAD_JR_PARSE);
    }
    ...
    try {
      ...
      if (schemaMalformed) {
        throw new ODKIncompleteSubmissionData(
            ...
            ODKIncompleteSubmissionData.Reason.ID_MALFORMED);
      } else {
        throw new ODKIncompleteSubmissionData(
            ...
            ODKIncompleteSubmissionData.Reason.ID_MISSING);
      }
    }
    if (!allowLegacy && rootElementDefn.modelVersion != null
      ...
      throw new ODKIncompleteSubmissionData(
          "The version string must be an integer less than 2147483648", ODKIncompleteSubmissionData.Reason.ID_MALFORMED);
    }
    ...
    if (!submissionElementDefn.equals(rootElementDefn)) {
      throw new ODKIncompleteSubmissionData(
          ...
          ODKIncompleteSubmissionData.Reason.MISMATCHED_SUBMISSION_ELEMENT);
    }
    ...
    if (base64RsaPublicKey == null || base64RsaPublicKey.length() == 0) {
      ...
      try {
        ...
        throw new ODKIncompleteSubmissionData("Exception " + e.toString() + " during parsing!", ODKIncompleteSubmissionData.Reason.BAD_JR_PARSE);
      }
      ...
      if (dataModel == null) {
        throw new ODKIncompleteSubmissionData(
            "Javarosa failed to construct Encrypted FormInstance!", ODKIncompleteSubmissionData.Reason.BAD_JR_PARSE);
      }
    ...
    if (formTitle == null) {
      if (existingTitle == null) {
        throw new ODKIncompleteSubmissionData(ODKIncompleteSubmissionData.Reason.TITLE_MISSING);
      } else {
